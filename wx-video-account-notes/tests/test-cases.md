...
3. Fresh machine path where `uv` is missing and bootstrap must download it, then continue into the normal flow: private runtime is created, required assets are prepared, and the output contains the material set.
